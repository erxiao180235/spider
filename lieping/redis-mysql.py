...
def main():
    #连接
    try:
        redis_cli = redis.StrictRedis(host='192.168.2.129', port=6379, db=0 ,password="123456")
        mysql_cli = MySQLdb.connect(host='192.168.2.129', user='root', passwd='123456', db='TESTDB', port=3306, charset='utf8')
        print "链接成功"
    except Exception,e:
    ...
    #操作
    while True:
        source, data = redis_cli.blpop(["liepin:items"])
        #将json转换为字典，进行数据存储
